...
echo "Starting ${NGPU}-GPU training on OpenDV (no language)..."
echo "Logs/checkpoints will be saved to: ${DST_PATH}"
echo "DINO_REPO=${DINO_REPO}"
echo "HF_HOME=${HF_HOME}"

# Language-annotated clip indexing without using language features.
torchrun --standalone --nnodes=1 --nproc_per_node="${NGPU}" --master_addr "${MASTER_ADDR}" --master_port "${MASTER_PORT}" /cpfs/pengyu/DINO-Foresight/train.py \
  ...
  --dst_path "${DST_PATH}" \
  --wandb_project dino-foresight --wandb_name opendv_mini_b16_6gpu_nolang \
  --wandb_mode online

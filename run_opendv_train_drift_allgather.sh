...
DST_ROOT="/cpfs/pengyu/DINO-Foresight/logs/dino_foresight_lowres_opendv_drift_allgather"
...
echo "Starting ${NGPU}-GPU drifting training on OpenDV (cross-rank negative pool)..."
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NGPU}" --master_addr "${MASTER_ADDR}" --master_port "${MASTER_PORT}" /cpfs/pengyu/DINO-Foresight/train.py \
  ...
  --dst_path "${DST_PATH}" \
  --wandb_project dino-foresight --wandb_name drift_small_fp16_ddp_allgather_b6a1 \
  --wandb_mode offline \

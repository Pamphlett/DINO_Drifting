...
DST_ROOT="/cpfs/pengyu/DINO-Foresight/logs/dino_foresight_lowres_opendv_residual_drift"
...
REGRESSION_CKPT="/cpfs/pengyu/DINO-Foresight/logs/dino_foresight_lowres_opendv_drift_noallgather/20260215_011651/checkpoints/epoch=9-step=535547.ckpt"

echo "Starting residual drifting finetune from regression checkpoint..."
echo "Checkpoint: ${REGRESSION_CKPT}"
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NGPU}" \
  ...
  --use_residual_drifting \
  --residual_queue_capacity 2048 \
  --residual_drift_weight 1.0 \
  --residual_drift_temperatures 0.02,0.05 \
  --drift_v_clip 20 \
  --drift_diversity_weight 0.1 \
  --drift_diversity_k 2 \
  --drift_noise_dim 256 \
  --context_mask_ratio 0.4 \
  --context_mask_schedule cosine_anneal \
  --context_mask_ratio_min 0.15 \
  ...
  --dst_path "${DST_PATH}" \
  --wandb_project dino-foresight --wandb_name residual_drift_finetune_from_regression \
  --wandb_mode online \

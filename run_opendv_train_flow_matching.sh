...
DST_ROOT="/cpfs/pengyu/DINO-Foresight/logs/dino_foresight_lowres_opendv_flow_matching"
...
echo "Starting ${NGPU}-GPU OpenDV training with conditional flow matching..."
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NGPU}" --master_addr "${MASTER_ADDR}" --master_port "${MASTER_PORT}" /cpfs/pengyu/DINO-Foresight/train.py \
  ...
  --single_step_sample_train --lr_base 8e-5 --gclip 2.0 \
  --use_flow_matching \
  --fm_num_steps 16 \
  --fm_time_embed_dim 256 \
  --fm_loss_weight 1.0 \
  --fm_train_on_last_frame_only true \
  --fm_noise_scale 1.0 \
  --fm_sampling_clip 0.0 \
  ...
  --dst_path "${DST_PATH}" \
  --wandb_project dino-foresight --wandb_name opendv_flow_matching \
  --wandb_mode online
